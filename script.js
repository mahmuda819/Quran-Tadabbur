...
async function syncToCloud(){
  ...
  try{
    ...
    setAuthStatus('Cloud sync failed — ' + detail);
    console.error('Cloud sync error details:', {message:e?.message, code:e?.code, details:e?.details, hint:e?.hint, status:e?.status});
    return false;
  }finally{
...
async function loadCloudData(user){
  ...
  try{
    ...
    const detail = e?.message || e?.details || e?.hint || 'Unknown error';
    setAuthStatus('Cloud load failed — ' + detail);
    console.error('Cloud load error details:', {message:e?.message, code:e?.code, details:e?.details, hint:e?.hint, status:e?.status});
    return 'error';
  }
}
